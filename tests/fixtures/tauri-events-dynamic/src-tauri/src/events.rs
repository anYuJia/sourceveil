...
/// A runtime-computed event name.
///
/// Nothing static can enumerate what this might be, so it could be any event
/// the application owns — including every one the frontend listens for.
pub fn wildcard<R: tauri::Runtime>(app: &AppHandle<R>, dynamic_event: &str) {
    app.emit(dynamic_event, 0u32).unwrap();
}
