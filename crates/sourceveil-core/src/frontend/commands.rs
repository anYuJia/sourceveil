//! `invoke` analysis: which command names the frontend asks for, and where a
//! call cannot be resolved to one.
//!
//! The unresolved case is the one that matters. `invoke(commandName)` computes
//! a command name at runtime, so it might be any command; renaming the Rust
//! handlers while such a call exists means the frontend asks for a name that no
//! longer exists. One dynamic call keeps the entire namespace.

use super::{collect_sources, line_of, parse, quote_at, slice, Span, StringLiteral};
use anyhow::Result;
...
pub fn analyze(root: &Path, extra_invoke_names: &[String]) -> Result<IpcAnalysis> {
    ...
    let (sources, read_warnings) = collect_sources(root);
    out.warnings.extend(read_warnings);
    let parsed_files: Vec<(PathBuf, String)> =
        sources.into_iter().map(|s| (s.path, s.text)).collect();
